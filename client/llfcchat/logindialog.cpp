    ...
    ui(new Ui::LoginDialog)
{
    ...
    //是设置 forget_label 标签的鼠标指针样式为手形光标
    ui->forget_label->setCursor(Qt::PointingHandCursor);
//将 forget_label 的 clicked 信号连接到 LoginDialog 类中的 slot_forget_pwd 槽函数
//forget_label是clicklable的实例
    connect(ui->forget_label, &ClickedLabel::clicked, this, &LoginDialog::slot_forget_pwd);
    initHttpHandlers();
    //连接登录回包信号
    //HttpMgr 中的登录完成信号连接到当前对象中的 slot_login_mod_finish 槽函数
...
void LoginDialog::initHttpHandlers()
{
    ...
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj){
        ...
        //发送信号通知tcpMgr发送长链接
        //构建 ServerInfo 对象，存储提取到的信息，并发出一个信号 sig_connect_tcp，用于启动与服务器的长连接。
        ServerInfo si;
...
void LoginDialog::slot_forget_pwd()
{
    ...
    emit switchReset();//发送信号给，mainwindows
}
...
bool LoginDialog::checkUserValid(){
    ...
    DelTipErr(TipErr::TIP_EMAIL_ERR);//移除错误
    return true;
}
